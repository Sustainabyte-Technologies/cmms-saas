  ...
  return (
    <section className="bg-slate-950 py-24 text-slate-50 border-b border-slate-800">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl backdrop-blur-xl overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow className="border-b border-slate-800 hover:bg-transparent">
                <TableHead className="w-[300px] text-base font-bold text-white">Capabilities & Features</TableHead>
                <TableHead className="text-center text-sm font-semibold text-slate-400">Excel / Paper Records</TableHead>
                <TableHead className="text-center text-sm font-semibold text-slate-400">Legacy Desktop CMMS</TableHead>
                <TableHead className="text-center text-base font-bold text-[#54EC46] bg-[#54EC46]/10 rounded-t-xl py-3">
                  FixByte AI-CMMS
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {comparisonData.map((row, idx) => (
                <TableRow key={idx} className="border-b border-slate-800/60 hover:bg-slate-900/40">
                  <TableCell className="font-semibold text-sm text-slate-200">{row.feature}</TableCell>
                  ...
                  <TableCell className="text-center">
                    {typeof row.excel === "boolean" ? (
                      ...
                      <span className="text-xs text-slate-400 font-medium">{row.excel}</span>
                    )}
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    {typeof row.legacy === "boolean" ? (
                      ...
                      <span className="text-xs text-slate-400 font-medium">{row.legacy}</span>
                    )}
                  </TableCell>

                  <TableCell className="text-center bg-[#54EC46]/5 font-bold">
                    {typeof row.fixbyte === "boolean" ? (
                      <CheckCircle2 className="mx-auto h-6 w-6 text-[#54EC46]" />
                    ) : (
                      <span className="text-xs text-[#54EC46] font-bold">{row.fixbyte}</span>
                    )}
                  </TableCell>
                </TableRow>
              ))}
